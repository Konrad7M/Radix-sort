		...
		for(i=1;i<=x;i++)
		{
			...
			if(sort.kom(i,10)->srt_n>=0)
			...
			else
				{
					...
					row.kom(x-column.kom((x+1),k)->srt_n,k)->srt_n=sort.kom(i,10)->srt_n;
					
					column.kom(x+1,k)->srt_n++;	
				}
...
//		cout<<"\n\n";
		...
		for(i=9;i>=0;i--)
		{
				for(d=1;d<=column.kom(x+1,i)->srt_n;d++)
				{					
					sort.kom(k++,10)->srt_n=row.kom(x+1-d,i)->srt_n;
				}
		}
...
switch(menu)
{
		...
		t_1.kom(b_1.MAX_X,10)->srt_n=++t_1.kom(b_1.MAX_X-1,10)->srt_n;
		
		t_1.kom(b_1.MAX_X-1,10)->srt_n=y;
